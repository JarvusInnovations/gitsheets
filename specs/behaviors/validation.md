...
- **Unknown-keyword leniency (changed from the pre-cutover `ajv` pass).** The former binding ran `ajv` in `strict: true` mode, which rejected _unknown JSON Schema keywords_ — and the non-standard `$data` extension — at compile with `ConfigError(config_invalid)`. The `jsonschema` crate is lenient: it silently ignores unknown keywords, so a schema with a typo'd keyword (or a `$data` reference) now compiles rather than failing. This is a deliberate, documented divergence introduced by the Rust-core cutover (see [`specs/rust-core.md`](../rust-core.md) and the core's `validation` module). Record-level validation of valid schemas is at parity with the former `ajv` pass (validity + instance path + failing-keyword `code`); only the strict-mode _schema-well-formedness_ gate is relaxed.
